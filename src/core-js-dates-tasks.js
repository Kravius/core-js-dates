...
function getNextFriday(date) {
  const dateNowWeek = new Date(date);
  const FridayNow = dateNowWeek.getDay();
  let countWeekNextFriday = 5 - FridayNow;
  if (countWeekNextFriday <= 0) countWeekNextFriday += 7;
  const friday = new Date(
    dateNowWeek.getTime() + countWeekNextFriday * 24 * 60 * 60 * 1000
  );
  return friday;
}
...
function isDateInPeriod(date, period) {
  ...
  const fined = new Date(date).getTime();

  return fined >= start && fined <= end;
}
...
function getQuarter(date) {
  const newDate = new Date(date).getUTCMonth();
  switch (true) {
    case newDate <= 2:
      return 1;
    case newDate > 2 && newDate <= 5:
      return 2;
    case newDate > 5 && newDate <= 8:
      return 3;
    default:
      return 4;
  }
